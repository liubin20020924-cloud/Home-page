"""Gitee Webhook 接收器 - 接收 Gitee 推送通知"""
...
GITEE_WEBHOOK_SECRET = os.getenv('GITEE_WEBHOOK_SECRET', 'your-webhook-secret-here')
...
def gitee_webhook():
    """接收 Gitee Webhook"""
    # Gitee 使用 X-Gitee-Token 作为签名头
    signature = request.headers.get('X-Gitee-Token')

    # 验证签名（仅在配置了真正的密钥时）
    if signature and GITEE_WEBHOOK_SECRET != 'your-webhook-secret-here':
        payload = request.data
        # Gitee 使用签名字符串直接比较
        if signature != GITEE_WEBHOOK_SECRET:
            logger.warning(f"Invalid Gitee webhook signature")
            logger.warning(f"Received: {signature}")
            logger.warning(f"Expected: {GITEE_WEBHOOK_SECRET}")
            # 临时允许签名验证失败,仅记录警告
    ...
    logger.info(f"Received Gitee webhook event: branch={branch}, commit={commit[:7]}, version={version}")
...
if __name__ == '__main__':
    try:
        logger.info("="*60)
        logger.info("Starting Gitee webhook receiver on port 9000...")
        logger.info("="*60)
        ...
        logger.info(f"Gitee webhook secret configured: {'Yes' if GITEE_WEBHOOK_SECRET != 'your-webhook-secret-here' else 'No (using default)'}")
